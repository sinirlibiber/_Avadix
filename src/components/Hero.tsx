...
const STATS = [
  ...
  { label: 'Markets', value: '48+', icon: TrendingUp },
  { label: 'Traders', value: '1,240+', icon: Shield },
  { label: 'On Avalanche', value: 'C-Chain', icon: Zap },
];
    ...
    <section style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '120px 24px 80px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 16, width: '100%', maxWidth: 700 }}>
        {STATS.map(({ label, value, icon: Icon }) => (
          <div key={label} style={{ background: 'rgba(18,18,26,0.8)', border: '1px solid rgba(30,30,46,0.8)', borderRadius: 16, padding: '20px 16px', backdropFilter: 'blur(10px)', transition: 'all 0.25s ease' }}
            ...
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 24, color: '#E2E2F0', lineHeight: 1, marginBottom: 4 }}>{value}</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#8888AA', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>{label}</div>
          </div>
        ))}
      </div>
