...
import { DONATION_QUOTES, DONATION_CAMPAIGNS, DonationCampaign } from '@/lib/data';
...
export default function DonationSection() {
  ...
  const [selected, setSelected] = useState<DonationCampaign>(DONATION_CAMPAIGNS[0]);
  const [amount, setAmount] = useState('');
  const [message, setMessage] = useState('');
  const [txStatus, setTxStatus] = useState<'idle' | 'success' | 'error'>('idle');
  const [showCreate, setShowCreate] = useState(false);
  const [form, setForm] = useState({ name: '', description: '', goal: '', emoji: '💎' });
  ...
  const handleDonate = () => {
    ...
    sendTransaction({ to: address!, value: parseEther(amount) }, {
      onSuccess: () => {
        setCampaigns(cs => cs.map(c => c.id === selected.id ? { ...c, raised: c.raised + parseFloat(amount), donors: c.donors + 1 } : c));
        setSelected(s => ({ ...s, raised: s.raised + parseFloat(amount), donors: s.donors + 1 }));
        setTxStatus('success');
  ...
  const handleCreate = () => {
    ...
    if (!form.goal || parseFloat(form.goal) <= 0) { setCreateError('Valid goal required.'); return; }
    ...
    setForm({ name: '', description: '', goal: '', emoji: '💎' });
    ...
    <section id="donate" style={{ padding: '80px 24px', maxWidth: 1280, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24 }}>
            ...
            {campaigns.map(camp => {
              ...
              return (
                <div key={camp.id} onClick={() => setSelected(camp)} style={{
                  background: isSel ? 'rgba(232,65,66,0.06)' : '#12121A',
                  border: `1px solid ${isSel ? 'rgba(232,65,66,0.35)' : '#1E1E2E'}`,
                  borderRadius: 14, padding: 16, cursor: 'pointer', transition: 'all 0.2s',
                }}>
                  <div style={{ display: 'flex', gap: 12, marginBottom: 10 }}>
                    <span style={{ fontSize: 28 }}>{camp.emoji}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 14, color: '#E2E2F0' }}>{camp.name}</div>
                      <div style={{ fontSize: 12, color: '#8888AA', marginTop: 2, lineHeight: 1.4 }}>{camp.description}</div>
                      {camp.creator && <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#555570', marginTop: 4 }}>by {camp.creator}</div>}
                    </div>
                  </div>
                  <div style={{ background: '#1E1E2E', borderRadius: 4, height: 6, overflow: 'hidden', marginBottom: 6 }}>
                    <div style={{ width: `${pct}%`, height: '100%', background: 'linear-gradient(90deg, #E84142, #ff6b6b)', borderRadius: 4, transition: 'width 0.8s ease' }} />
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#E84142' }}>{pct}% · {camp.donors} donors</span>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#8888AA' }}>{camp.raised.toLocaleString()} / {camp.goal.toLocaleString()} AVAX</span>
                  </div>
                </div>
              );
            })}
        ...
        <div style={{ background: '#12121A', border: '1px solid #1E1E2E', borderRadius: 20, padding: 28, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {txStatus === 'error' && <div style={{ padding: '10px', background: 'rgba(232,65,66,0.1)', border: '1px solid rgba(232,65,66,0.2)', borderRadius: 8, color: '#E84142', fontSize: 13, fontFamily: 'var(--font-mono)', textAlign: 'center' }}>Transaction failed. Try again.</div>}
          ...
          {txStatus === 'success' ? (
            <div style={{ padding: 16, background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 10, textAlign: 'center', color: '#22c55e', fontFamily: 'var(--font-display)', fontWeight: 600 }}>
              💚 Thank you! Your donation has been sent!
            </div>
          ) : (
            <button onClick={handleDonate} disabled={isPending} style={{
              width: '100%', padding: '14px',
              background: isConnected ? '#E84142' : '#2A2A3E',
              ...
              {isPending ? 'Processing...' : !isConnected ? 'Connect Wallet to Donate' : `Donate ${amount ? amount + ' AVAX' : 'Now'}`}
              {isConnected && !isPending && <Sparkles size={14} />}
            </button>
          )}
      ...
      {showCreate && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(6px)', padding: 24 }}>
          <div style={{ background: '#12121A', border: '1px solid #1E1E2E', borderRadius: 20, padding: 32, width: '100%', maxWidth: 480, position: 'relative' }}>
            ...
            {createSuccess ? (
              <div style={{ padding: 20, background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: 12, textAlign: 'center', color: '#22c55e', fontFamily: 'var(--font-display)', fontWeight: 600 }}>✓ Campaign created!</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div>
                  ...
                  <input type="number" placeholder="e.g. 500" value={form.goal} onChange={e => setForm(f => ({ ...f, goal: e.target.value }))} style={{ width: '100%', padding: '12px 14px', background: '#0A0A0F', border: '1px solid #1E1E2E', borderRadius: 10, color: '#E2E2F0', fontFamily: 'var(--font-mono)', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} />
                </div>
                ...
                <button onClick={handleCreate} style={{ width: '100%', padding: '13px', background: '#E84142', border: 'none', borderRadius: 10, color: 'white', fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 15, cursor: 'pointer', boxShadow: '0 0 20px rgba(232,65,66,0.3)' }}>
                  {!isConnected ? '⚠ Connect Wallet First' : 'Launch Campaign'}
                </button>
              </div>
            )}
          </div>
